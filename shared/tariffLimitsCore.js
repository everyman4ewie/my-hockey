...
export function getTariffLimits(tariffId) {
  const key = getLimitsLookupKey(tariffId)
  return TARIFF_LIMITS[key] || TARIFF_LIMITS.free
}

/**
 * Проверка лимитов (как на сервере): для createPlan учитывается plansMonthKey.
 */
...
export function getLimitLabel(tariffId, action) {
  ...
  switch (action) {
    case 'createPlan':
      return limits.maxPlansPerMonth < 0 ? null : limits.maxPlansPerMonth
    case 'downloadPdf':
      return limits.maxPdfDownloads < 0 ? null : limits.maxPdfDownloads
    case 'downloadWord':
      return limits.maxWordDownloads < 0 ? null : limits.maxWordDownloads
    case 'downloadBoard':
      return limits.maxBoardDownloads < 0 ? null : limits.maxBoardDownloads
    case 'maxExercisesPerPlan':
      return limits.maxExercisesPerPlan < 0 ? null : limits.maxExercisesPerPlan
    default:
      return null
  }
}
